...
public class StepNode
{
	...
	public void validade(File file, String scenarioName)
	{
		...
		if (StringUtils.isNoneBlank(getName(), getReference(), getExecutor()))
			throw new AttributeException(defaultError + "To create an step is necessary choice OR fill the attributes '%s' AND '%s' OR the attribute '%s'", ATTR_NAME,
					ATTR_REFERENCE, ATTR_EXECUTOR);
